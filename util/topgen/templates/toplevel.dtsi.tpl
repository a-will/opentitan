 ...
 */
%   for name, value, docstring in helper.pinmux_peripheral_in.constants:
/** ${docstring} */
 ...
 */
%   for name, value, docstring in helper.pinmux_insel.constants:
/** ${docstring} */
#define ${name.as_c_define()} ${value}
%   endfor
% endif
 ...
 */
%   for name, value, docstring in helper.pinmux_mio_out.constants:
/** ${docstring} */
#define ${name.as_c_define()} ${value}
%   endfor
% endif
 ...
 */
%   for name, value, docstring in helper.pinmux_outsel.constants:
/** ${docstring} */
#define ${name.as_c_define()} ${value}
%   endfor
% endif
...
<%
    ...
    unit_name = f"memory@{region.base_addr:x}"
%>\
    /**
...
<%
  ...
  devices = helper.devices(addr_space)
  interrupts = {}
...
%>\
% for m in top["module"]:
%   if m["name"] in helper.device_regions[addr_space]:
<%
  inst_name = m["name"]
  inst_type = m["type"]
  
  device_regions = helper.device_regions[addr_space][inst_name]
  # Do our best to get the primary address / interface
  ...
  elif 'regs' in device_regions:
    base_addr = device_regions['regs'].base_addr
  else:
    sorted_keys = sorted(device_regions.keys())
    base_addr = device_regions[sorted_keys[0]].base_addr
  regs = []
  reg_names = []
  for reg_name, reg in device_regions.items():
    regs.append(reg)
    reg_names.append(reg_name)
  unit_name = f"{inst_type}@{base_addr:x}"
  base_addr = "{:08x}".format(base_addr)
%>\
        ${inst_name}: ${unit_name} {
