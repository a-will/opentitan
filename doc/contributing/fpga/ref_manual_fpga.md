...
For example, see the `//hw/top_earlgrey/bitstream:rom` target defined in [hw/top_earlgrey/bitstream/BUILD](https://github.com/lowRISC/opentitan/blob/master/hw/top_earlgrey/bitstream/BUILD).
...
The `chip_earlgrey_cw310` CI job builds the `//hw/top_earlgrey/bitstream/vivado:standard` target which will build a bitstream with the test ROM and RMA OTP image.
...
One meta-level up, we have targets in `//hw/top_earlgrey/bitstream` that decide whether to use cached artifacts or to build them from scratch.
